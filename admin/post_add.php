    ...
    <div class="posts">
      <div class="post">
        <form action="./post_add_handle.php" method="POST">
          ...
          <div class="select post__category">category : <select class='post__category' name='category_id'>
          <?php 
            foreach($results as $result){
              //將查詢出的資料輸出
              $categoryId = $result['ID'];
              $categoryName = $result['category_name'];
              echo "<option class='post__category'  value='$categoryId'>" . $categoryName . "</option>";
              // echo var_dump($categoryId);
            }
          ?>
          </select></div>
          <div class="select post__status">Post status:
            <select class='post__status' name='post_status'>
